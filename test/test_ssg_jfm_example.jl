...
#
# Usage: 
#   julia test_ssg_jfm_example.jl                    # Single process
#   mpirun -np 4 julia test_ssg_jfm_example.jl      # Multi-process
...
function create_jfm_domain(Nx::Int, Ny::Int, Nz::Int; 
    ...
    if rank == 0
        println("🌊 JFM Domain Configuration:")
...
function compute_jfm_diagnostics(fields::Fields{T}, domain::Domain) where T
    ...
    rfft_2d!(domain, view(fields.u, :, :, size(fields.u, 3)), fields.tmpc_2d)
    ddx_2d!(domain, fields.tmpc_2d, fields.tmpc2_2d)
    irfft_2d!(domain, fields.tmpc2_2d, fields.tmp)
    dudy = copy(fields.tmp.data)
    
    rfft_2d!(domain, view(fields.v, :, :, size(fields.v, 3)), fields.tmpc_2d)  
    ddy_2d!(domain, fields.tmpc_2d, fields.tmpc2_2d)
    irfft_2d!(domain, fields.tmpc2_2d, fields.tmp2)
    dvdx = copy(fields.tmp2.data)
...
function run_jfm_test_simulation(Nx::Int=64, Ny::Int=64, Nz::Int=16; 
    ...
    try
        ...
        if rank == 0
            println("\n⏱️  Starting time integration ($n_steps steps)...")
        end
