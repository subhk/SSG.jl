...
function solve_monge_ampere_fields!(fields::Fields{T}, 
    ...
    solution, diagnostics = solve_ssg_equation(fields.φ, b_3d, ε, domain; 
                                             tol=tol, 
                                             maxiter=maxiter,
                                             verbose=verbose,
                                             smoother=:spectral)
...
# Export main solver functions for use with existing codebase
# These maintain compatibility with nonlinear.jl, timestep.jl, etc.

"""
## SSG EQUATION (A1) IMPLEMENTATION SUMMARY:
...
where:
- ∇² = ∂²/∂X² + ∂²/∂Y² + ∂²/∂Z²            (3D Laplacian)
- DΦ = ∂²Φ/∂X²∂Y² - (∂²Φ/∂X∂Y)²            (A2, nonlinear operator)
